...
class InlineTrackerBlock(nn.Module):
    ...
    def forward(self, img_feat, event_feat):
        ...
        concat_features = torch.cat([warped_feat, img_feat], dim=1)
        integration_weights = self.integration_gate(concat_features)
        output = warped_feat * integration_weights + img_feat * (1 - integration_weights)
...
class EFNet_AttTracking_CABFusion_enhanced(nn.Module):
    def __init__(
        self,
        in_chn=3,
        ev_chn=6,
        ...
        num_heads=[1, 2, 4],
        use_tracking=True,
        use_multiscale_tracking=True,
    ):
        super(EFNet_AttTracking_CABFusion_enhanced, self).__init__()
        self.depth = depth
        self.fuse_before_downsample = fuse_before_downsample
        self.num_heads = num_heads
        self.use_tracking = use_tracking
        self.use_multiscale_tracking = use_multiscale_tracking
        ...
        # Create a list of channel dimensions for each scale
        self.scale_channels = [(2**i) * wf for i in range(depth)]
        
        # Create multi-scale tracking module if enabled
        if use_tracking and use_multiscale_tracking:
            self.multiscale_tracker = MultiScaleTrackingModule(self.scale_channels)

        prev_channels = self.get_input_chn(wf)
        for i in range(depth):
            downsample = True if (i + 1) < depth else False

            # Enable tracking for all layers when using multi-scale tracking
            layer_use_tracking = use_tracking and (i in [0, 1] or use_multiscale_tracking)
    ...
    def forward(self, x, event, mask=None):
        ...
        for i, down in enumerate(self.down_path_1):
            if (i + 1) < self.depth:
                ...
                if self.use_tracking and hasattr(down, 'feature_tracker'):
                    tracked_features.append(x1)
        
        # Apply multi-scale tracking if enabled
        if self.use_tracking and self.use_multiscale_tracking and tracked_features:
            enhanced_features = self.multiscale_tracker(tracked_features)
            
            for i in range(len(enhanced_features) - 1):
                encs[i] = enhanced_features[i]
            x1 = enhanced_features[-1]
    ...
    def reset_lstm_states(self):
        for i in range(self.depth):  # Reset all layers when using multi-scale tracking
...
class CoarseToFineFusionModule(nn.Module):
    def __init__(self, feat_channels=64, reduction=16):
        ...
        self.fusion_block_1 = ChannelAttentionBlock(
            n_feat=feat_channels * 2, reduction=reduction, act=nn.ReLU(True)
        )

        self.fusion_block_2 = ChannelAttentionBlock(
            n_feat=feat_channels * 2, reduction=reduction, act=nn.ReLU(True)
        )
...
class MultiScaleTrackingModule(nn.Module):
    def __init__(self, channels_list):
        super(MultiScaleTrackingModule, self).__init__()
        self.channels_list = channels_list
        self.num_scales = len(channels_list)
        
        # Downscale projections (from higher resolution to lower)
        ...
        # Upscale projections (from lower resolution to higher)
        self.up_projections = nn.ModuleList()
        for i in range(self.num_scales - 1):
            self.up_projections.append(nn.Sequential(
                nn.ConvTranspose2d(channels_list[i+1], channels_list[i], kernel_size=4, stride=2, padding=1),
                nn.LeakyReLU(0.2, inplace=False)
            ))
        ...
        for i in range(self.num_scales):
            # For each scale, we might receive features from adjacent scales
            num_inputs = 1  # Original features
            if i > 0:  # Can receive from scale below
                num_inputs += 1
            if i < self.num_scales - 1:  # Can receive from scale above
                num_inputs += 1
                
            self.fusion_convs.append(nn.Sequential(
                nn.Conv2d(channels_list[i] * num_inputs, channels_list[i], kernel_size=3, padding=1),
                nn.LeakyReLU(0.2, inplace=False)
            ))
    
    def forward(self, tracked_features):
        # tracked_features is a list of features from different scales
        # Each element has been processed by its respective InlineTrackerBlock
        
        # First pass: downscale features from higher to lower resolution
        downscaled_features = [tracked_features[0]]
        for i in range(self.num_scales - 1):
            downscaled = self.down_projections[i](downscaled_features[-1])
            downscaled_features.append(downscaled)
        
        # Second pass: upscale features from lower to higher resolution
        upscaled_features = [None] * self.num_scales
        upscaled_features[-1] = tracked_features[-1]
        for i in range(self.num_scales - 2, -1, -1):
            upscaled = self.up_projections[i](upscaled_features[i+1])
            upscaled_features[i] = upscaled
        
        # Final pass: fuse features at each scale
        output_features = []
        for i in range(self.num_scales):
            to_fuse = [tracked_features[i]]  # Original tracked features
            
            if i > 0 and downscaled_features[i] is not None:
                to_fuse.append(downscaled_features[i])
                
            if i < self.num_scales - 1 and upscaled_features[i] is not None:
                to_fuse.append(upscaled_features[i])
                
            fused = self.fusion_convs[i](torch.cat(to_fuse, dim=1))
            output_features.append(fused)
...
if __name__ == "__main__":
    ...
    model = EFNet_AttTracking_CABFusion_enhanced()
