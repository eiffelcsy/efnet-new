...
class EFNet_att_track_freq_cabfusion_modified(nn.Module):
    def __init__(
        self,
        in_chn=3,
        ev_chn=6,
...
class UNetConvBlock(nn.Module):
    def __init__(
        ...
        self.freq_attention_1 = FCFE(out_size) # added here
        self.freq_attention_2 = FCFE(out_size) # added here
    ...
    def forward(
        ...
        if self.num_heads is not None and event_filter is not None:
            if merge_before_downsample:
                out_clone = out.clone() # added here
                out = self.image_event_transformer(out, event_filter)
                out = out + self.freq_attention_1(event_filter, out_clone) # added here
        ...
        if self.downsample:
            ...
            if not merge_before_downsample and self.num_heads is not None and event_filter is not None:
                out_down_clone = out_down.clone() # added here
                out_down = self.image_event_transformer(out_down, event_filter)
                out_down = out_down + self.freq_attention_2(event_filter, out_down_clone) # added here
            return out_down, out
        else:
            if not merge_before_downsample and self.num_heads is not None and event_filter is not None:
                out_clone = out.clone() # added here
                out = self.image_event_transformer(out, event_filter)
                out =  out + self.freq_attention_2(event_filter, out_clone) # added here
            return out
...
if __name__ == "__main__":
    ...
    model = EFNet_att_track_freq_cabfusion()
